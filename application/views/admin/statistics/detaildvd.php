...
<!-- /////////////////////////////// -->
      <section class="sec01">
        <h3 class="tt_main">DVD在庫を集計する</h3>
        <div class="research_box">
          <form method="post" id="search_parent" action="<?php echo base_url().'admin/statistics/dvd'  ?>">
            <table class="formtable">
              <tr>
                <th>イベントID</th>
                <td>
                  <select name="event" id="event">
                    <option id ="event_current"   value="<?php if(isset($id_event_input)){
                        echo $id_event;
                    } ?>"><?php if(isset($id_event_input)){
                        echo $id_event_input;
                    } ?></option>

                    <?php foreach ($list_event as  $row_list_event) {
                     ?>
                    <option value="<?=$row_list_event['lm10_event_id']?>"><?=$row_list_event['lm10_event_id_input']?></option>
                    <?php } ?>
                  </select>
                </td>
              </tr>
              <tr>
                <th>イベント名</th>
                <td id="name_event"><?php if(isset($id_event_input)){
                        echo $name_event;
                    } ?></td>
              </tr>
            ...
            <div class="search_cf">
              <p>
                <input type="submit" name="btn_search" class="btn btn_search" value="集計" id="btn_search">
              </p>
            </div>
          </form>
        </div>
          ...
          <table class="table02">
           ...
           <tbody>
            <?php if(isset($is)&&$is>0){ ?>
              <?php foreach ($L_product as $aListP ) {
...
<!-- isset type and active -->
                ...
                ?></td>
                <td >
                  <button  id_pro = '<?=$bListP['lm04_pro_id']; ?>' cr_qu = '<?=$bListP['lm04_pro_quantity']; ?>' event_id ='<?=$aListP['id_event_input']?>' event_name ='<?=$aListP['name_event']?>' product_name="<?=$bListP['lm04_pro_name']?>" class="btn01">追加する</button></td>
              </tr>
               <?php } } } else{ ?>
...
<script type="text/javascript">
  ...
  $('.static_detail_dvd').addClass('active');
</script>
<!-- click get name event -->
<!-- get event by year -->
<script>
  ...
  $("#event").change(function(){
   ...
   if(event != ""){
    $.post("<?php echo base_url()?>admin/statistics/getEvent_byId",{event:event}).done(function(data){
          $("#name_event").html(data);
          });
   }else{
    $("#name_event").html('');
   }
...
<script>
  $(document).ready(function() {
    ...
    $('#add_stock_form').submit(function(event){
      ...
      if(quanlity_input > 0){
        $.ajax({
          url:"<?=base_url()?>/admin/statistics/addStockDVD",
          method:"POST",
